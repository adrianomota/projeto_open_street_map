...
Um outro desafio é a criação de aplicações geocoders para a inclusão de informações precisas no dataset do openstreetmap. Um exemplo seria eu ir aos quatro supermercados da minha região analisar os preços das cestas básicas tirar uma média e publicar essa informação no openstretmap, isso com certeza fomentaria uma guerra comrcial entre os estabelecimentos e estimularia a competitividade entre eles e o consumidor sairia ganhando, porém isso geraria um outro desafio que seria a criação de uma camada de API para essas aplicações.
Um outrovcenário é a criação de QRCode especificos para inclusão de informações no openstreetmap, no brasil isso poderia ser feito para o senso do IBGE. Tags assinadas digitalmente garantiria a confiabilidade das informações no openstreetmap.
